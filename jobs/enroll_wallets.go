...
func (job EnrollWalletsJob) Run() {
	...
	tx := job.DB.Model(&models.SentinelPlanSubscription{}).Order("id desc").First(&sentinelPlanSubscription, "inactive_at > ?", time.Now())
	if tx.Error != nil {
	...
	tx = job.DB.Model(&models.Device{}).Order("created_at").Limit(100).Where("subscription_id IS DISTINCT FROM ? AND current_balance > 0", sentinelPlanSubscription.ID).Find(&devices)
	if tx.Error != nil {
