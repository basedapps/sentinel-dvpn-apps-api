...
func (job EnrollWalletsJob) Run() {
	...
	tx = job.DB.Model(&models.Device{}).Order("created_at").Limit(10).Where("subscription_id IS DISTINCT FROM ? AND current_balance > 0", sentinelPlanSubscription.ID).Find(&devices)
	if tx.Error != nil {
