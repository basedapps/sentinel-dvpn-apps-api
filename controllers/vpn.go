...
func (vc VPNController) ConnectToCity(c *gin.Context) {
	...
	if protocol != "" {
		err = vc.DB.Order("id").First(&server, "current_load < ? AND country_id = ? AND city_id = ? AND is_included_in_plan = ? AND is_banned = ? AND is_active = ? AND protocols->>0 = ?", 0.9, countryId, cityId, true, false, true, protocol).Error
		if err != nil {
	...
	err = vc.DB.Order("id").First(&server, "current_load < ? AND country_id = ? AND city_id = ? AND is_included_in_plan = ? AND is_banned = ? AND is_active = ?", 0.9, countryId, cityId, true, false, true).Error
	if err != nil {
		middleware.RespondErr(c, middleware.APIErrorUnknown, "failed to get server: "+err.Error())
		return
	}
